            ...
            <thead>
              <tr>
                <th class="center" width="40%">
                  <input id="name" name="name" placeholder="Core Material" class="form-control">
                </th>
                <th class="center" width="40%">
                ...
                <th class="center"><button class="btn btn-primary" id="addCore" type="submit">Add</button></th>
                <th class="center"></th>
              </tr>
              <tr>
                ...
                <th class="center" width="30%" style="vertical-align:middle">
                  <input id="searchName" name="name" placeholder="Core Material" class="form-control">
                </th>
                <th class="center" width="30%" style="vertical-align:middle">
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $(document).delegate(".deleteCore","click",function(e){
      e.preventDefault();
      var data = {};
      data['coreId'] = $(this).attr('coreId');
      $('#myModal2').modal('show');
      $.Mustache.load('template/updateCore.mustache', function() {
        $('#modalBody').html('');
        $('#modalTittle').html('Are you sure you want to delete?');
        $('#modalFooter').html('<button data-dismiss="modal" class="btn btn-primary" type="button">Close</button><button class="btn btn-danger" type="button" id="deleteCore"> Confirm</button>');
        $('#deleteCore').click(function(e){
          e.preventDefault();
          data['_op'] = 'delete_core';
          handler('backend/processing.php',data,update,'');
        });
      });
    });
    
    $(document).delegate(".editCore","click",function(e){
      e.preventDefault();
      var data = {};
      data['coreId'] = $(this).attr('coreId');
      ...
      $.Mustache.load('template/updateCore.mustache', function() {
        ...
        $('#modalTittle').html('Modify Core Information');
        $('#modalFooter').html('<button data-dismiss="modal" class="btn btn-primary" type="button">Close</button><button class="btn btn-danger" type="button" id="updateCore"> Update</button>');
        $('#updateCore').click(function(e){
          e.preventDefault();
          var name = $('#name').val();
  ...
  function list(data){
    ...
    $('#list').html('');
    $.Mustache.load('template/core.mustache', function() {
