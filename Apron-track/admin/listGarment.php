            ...
            <thead>
              <tr>
                <th class="center" width="40%">
                  <input id="name" name="name" placeholder="Garment" class="form-control">
                </th>
                <th class="center" width="40%">
                ...
                <th class="center"><button class="btn btn-primary" id="addGarment" type="submit">Add</button></th>
                <th class="center"></th>
              </tr>
              <tr>
                ...
                <th class="center" width="30%" style="vertical-align:middle">
                  <input id="searchName" name="name" placeholder="Garment" class="form-control">
                </th>
                <th class="center" width="30%" style="vertical-align:middle">
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#addGarment').click(function(e){
      ...
      var data = {};
      data['_op'] = 'add_garment';
      data['name'] = name;
      data['status'] = status;
      handler('backend/processing.php',data,add,'');
    });

    $(document).delegate(".deleteGarment","click",function(e){
      e.preventDefault();
      var data = {};
      data['garmentId'] = $(this).attr('garmentId');
      $('#myModal2').modal('show');
      $.Mustache.load('template/updateGarment.mustache', function() {
        $('#modalBody').html('');
        $('#modalTittle').html('Are you sure you want to delete?');
        $('#modalFooter').html('<button data-dismiss="modal" class="btn btn-primary" type="button">Close</button><button class="btn btn-danger" type="button" id="deleteGarment"> Confirm</button>');
        $('#deleteGarment').click(function(e){
          e.preventDefault();
          data['_op'] = 'delete_garment';
          handler('backend/processing.php',data,update,'');
        });
      });
    });
    
    $(document).delegate(".editGarment","click",function(e){
      e.preventDefault();
      var data = {};
      data['garmentId'] = $(this).attr('garmentId');
      ...
      $.Mustache.load('template/updateGarment.mustache', function() {
        ...
        $('#modalTittle').html('Modify Garment Information');
        $('#modalFooter').html('<button data-dismiss="modal" class="btn btn-primary" type="button">Close</button><button class="btn btn-danger" type="button" id="updateGarment"> Update</button>');
        $('#updateGarment').click(function(e){
          e.preventDefault();
          var name = $('#name').val();
  ...
  function list(data){
    ...
    $('#list').html('');
    $.Mustache.load('template/garment.mustache', function() {
